 ...
 *          btSubmit (Button)
 */
public class PrefixDetector extends LayoutDetector {
    public static final Issue ISSUE = Issue.create(
            "ViewId",
            "ID non valido",
            "ID deve avere un prefisso contenente l'abbreviazione della classe.\n"
                    + "Ex: TextView -> tvMyTextView",
    ...
    private void report(XmlContext context, Element element, String type, String viewId,
        ...
        context.report(ISSUE, context.getLocation(element),
                String.format("View '%s' del tipo %s deve avere come prefisso '%s'", viewId, type,
                        expectedPrefix));
    }
}